...
NOTE: The plugin will try to maintain a single event per date and group of metrics, in order to prevent inaccurate results when creating visualisations due to duplication. To achieve this, the event field `[@metadata][id]` is set to the date plus metric names. You'll need to add the https://www.elastic.co/guide/en/logstash/current/plugins-outputs-elasticsearch.html#plugins-outputs-elasticsearch-document_id[`document_id`] option in your Elasticsearch output to `"%{[@metadata][id]}"` to make use of this.
...
"ga.query": "{\"metrics\":[\"ga:users\",\"ga:sessions\"],\"dimensions\":\"ga:browser,ga:city\",\"ids\":\"ga:976488984\",\"max_results\":1000,\"start_date\":\"2020-04-06\",\"end_date\":\"2020-04-06\",\"start_index\":1}",
"ga.date": "2020-04-07",
"[@metadata][id]": "2020-04-07ga:sessions,ga:users",
"@version": "1"
}
-----
